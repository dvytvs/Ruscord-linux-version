# Russcord

Russcord - не официальный десктопный клиент для Linux  
Приложение для общения в формате серверов, каналов, текста и голоса

Это самостоятельный продукт со своей экосистемой и развитием внутри проекта Russcord

---

## ✨ О продукте

Russcord даёт привычный опыт общения в виде отдельного приложения

Вы получаете

💬 привычные серверы и каналы  
⚡ отдельное приложение вместо браузера  
🔄 стабильные обновления и развитие  
🏷 единый продукт с собственным брендом  

---

## 👥 Для кого это

Russcord подойдёт если вы

🧠 создаёте своё сообщество  
📦 хотите отдельное приложение под проект  
🚫 не хотите зависеть от сторонних клиентов  
🎯 цените простой и понятный интерфейс  

---

## 📸 Интерфейс
<img width="1915" height="1038" alt="Снимок экрана от 2026-05-03 19-33-13" src="https://github.com/user-attachments/assets/0afbc6c1-b947-44c8-8957-56719dc4771b" />

## 📥 Установка

Скачайте последнюю версию 
 Есть версии для всех дистрибутивов. Для удобства есть flatpak и AppImage

Рекомендуется скачивать только с официальных релизов

👉 https://github.com/ruscord/ruscord-electron-communities/releases

Но конкретно этот именно от сюда. Версия для ARM зедесь - https://github.com/ruscord/russcord-app.git

---

## ⚠️ Если что-то не работает

Пожалуйста, проверьте права на запуск программы. Если же не работает, отправьте запуск из терминала в Issues и я вам обязательно помогу.

---

## ℹ️ Важно

Russcord - независимое приложение  
Оно не связано с другими платформами и развивается как отдельный продукт

## 📄 Лицензия

Проект распространяется под лицензией GNU GENERAL PUBLIC LICENSE, а именно GPLv3


## ⚠️  ВНИМАНИЕ!!!

Разработчик dvytvs не берёт на себя ответственность за работу самого сервиса. Все вопросы в поддержку самого Ruscord - @supruscord_bot

Канал самого Ruscord - https://t.me/russcord
